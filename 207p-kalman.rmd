...
```{r}
## Kalman Filtering
split_df <- ts_split(ts.obj = arkg_ts, sample.out = 20)

training = split_df$train

model <- function(p) {
  return(dlmModPoly(2, dV = p[1], dW = p[2:3]) + dlmModSeas(20, dV = p[4]))
}
...
x <- index(training)
xf <- seq(max(x), max(x) + n/124, 1/124)
aa <- forecast$a[,1]* (-1)
aa <- cbind(forecast$a[,1], a)
